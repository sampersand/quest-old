...
macro_rules! create_token {
	...
	($name:ident oper binary $oper:ty, $regex:path, $binding:expr) => {{
		use obj::classes::opers::*;
		use self::Binding::*;
		Token {
			name: stringify!($name),
			binding: $binding,
			match_fn: normal_match_fn!($regex),
			qobj: Some(|tree, env| QObject::from(<$oper>::from_tree(tree)).call(&[], env) ),
			..Token::default()
		}
	}};
}

fn object_from_tree<T: FromStr<Err=E> + Into<QObject>, E: Debug>(tree: &Tree) -> QObject {
...
tokens! {
	...
	token B_TRUE(object QBool, bool::RE_TRUE);
	...
	token V_NORM {
		match_fn: normal_match_fn!(classes::var::REGEX),
		qobj: Some(|tree, env| {
