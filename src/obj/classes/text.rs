...
impl Parsable for String {
	fn try_parse(env: &mut Environment) -> Option<Token> {
		// double quotes
		if let Some(text) = env.stream.try_get(regex!(r#"\A("(\\.|[^"])*")"#)) {
		...
		// grave quote
		if env.stream.as_str().chars().next() == Some('`') {
...
define_attrs! { for QText;
	...
	fn "@num" (this) {
		// let mut env: Stream = Stream::from_str(this.as_ref());
		let mut env = unimplemented!();
		unimplemented!("TODO: @num for text");
		Ok(QNum::try_parse(&mut env).expect("Invalid number given (todo: make this an exception)"))
	}
