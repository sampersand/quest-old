...
macro_rules! impl_from_builtin {
	($($ty:ident)*) => {
		$(
			...
			impl From<$ty> for QObject__ {
				#[inline]
				fn from(num: $ty) -> QObject__ {
					QNum::from(num).into()
				}
			}
...
fn into_qnum(pos: &QObject__, env: &Environment__) -> QNum {
	pos.as_num(env).expect("`@num` is required to interact with `QNum`")
}

fn into_f64(pos: &QObject__, env: &Environment__) -> f64 {
...
default_attrs! { for QNum, with variant Num;
	...
	fn "@bool" (this) {
		Ok(::obj_::QObject_::Old((this.0 != 0.0).into()))
	}
	...
	fn "@text" (this) {
		if let Some(int) = this.try_to_int() {
			Ok(::obj_::QObject_::Old(int.to_string().into()))
		} else {
			Ok(::obj_::QObject_::Old(this.0.to_string().into()))
		}
	}
	...
	fn "round" (this; places = ().into()) with env {
		...
		Ok(::obj_::QObject_::Old(this.0.round().into()))
	}

	fn "abs" (this) {
		Ok(::obj_::QObject_::Old(this.0.abs().into()))
	}

	fn "+" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0 + into_f64(rhs, env)).into())) }
	fn "-" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0 - into_f64(rhs, env)).into())) }
	fn "*" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0 * into_f64(rhs, env)).into())) }
	fn "/" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0 / into_f64(rhs, env)).into())) }
	fn "^" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0.powf(into_f64(rhs, env))).into())) }
	fn "%" (this, rhs) with env { Ok(::obj_::QObject_::Old((this.0 % into_f64(rhs, env)).into())) }
	...
	fn "<=>" (this, rhs) with env {
		...
		if this.0 < other {
			Ok(::obj_::QObject_::Old((-1.0).into()))
		} else if this.0 == other {
			Ok(::obj_::QObject_::Old(0.0.into()))
		} else {
			Ok(::obj_::QObject_::Old(1.0.into()))
		}
	}
