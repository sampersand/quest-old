...
macro_rules! default_attrs {
	(for $for:ty; $(use $what:ident;)* $(fn $attr:tt $args:tt $($with_stuff:ident)* $body:block)*) => {
		impl ::obj_::attrs::HasDefaultAttrs for $for {
...
pub use self::object::{Id, IdType, QObject_, QObject__, Classes};
pub use self::exception::{Exception__, Result__, Result_, Exception_};